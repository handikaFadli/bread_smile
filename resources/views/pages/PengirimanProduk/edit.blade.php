    ...
    <div class="intro-y col-span-12 lg:col-span-8">
        ...
        <div class="intro-y box px-10 py-5">
            <form class="" action="{{ route('mobil.update', $mobil->kd_mobil) }}" method="POST">
                @csrf
                @method('PUT')
                <input id="kd_mobil" name="kd_mobil" type="hidden" value="{{ $mobil->kd_mobil }}">
                <div class="bg-slate-100 -ml-1 w-full flex justify-center py-2 rounded-lg shadow-lg">
                    <span class="text-slate-800 font-medium uppercase text-center">Kode Mobil - {{ $mobil->kd_mobil }}</span>
                </div>
                <div class="mt-8">
                    <label for="merk" class="form-label"> Merk Mobil </label>
                    <input name="merk" id="merk" type="text" class="form-control w-full shadow-md @error('merk') border-danger @enderror" value="{{ old('merk', $mobil->merk) }}">
                    @error('merk')
                ...
                <div class="mt-6">
                    ...
                    <input name="plat_nomor" id="plat_nomor" type="text" class="form-control w-full shadow-md @error('plat_nomor') border-danger @enderror" value="{{ old('plat_nomor', $mobil->plat_nomor) }}">
                    @error('plat_nomor')
                        <div class="text-danger mt-1">
                            {{ $message }}
                        </div>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="ket" id="ket" class="form-control w-full shadow-md @error('ket') border-danger @enderror">{{ old('ket', $mobil->ket) }}</textarea>
                    @error('ket')
                        <div class="text-danger mt-1">
                            {{ $message }}
                        </div>
