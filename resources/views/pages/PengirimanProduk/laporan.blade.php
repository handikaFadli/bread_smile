...
<body @isset($print) onload="javascript:window.print()" @endisset>
    ...
    <div class="body">
        ...
        <table cellpadding="0" cellspacing="0">
            ...
            <tr>
                <td>{{ $loop->iteration }}</td>
                <td>{{ $dt->nm_produk }} ({{ $dt->jumlah }} Pcs)</td>
                <td>{{ $dt->created_at->isoFormat('dddd, D MMM Y') }}</td>
                <td>{{ $dt->updated_at->isoFormat('dddd, D MMM Y') }}</td>
                <td>{{ $dt->nm_sopir }} ({{ $dt->plat_nomor }})</td>
                <td>{{ $dt->tempat.', '. $dt->alamat}}</td>
                <td>{{ $dt->nm_penerima }}</td>
            </tr>
            @endforeach
        </table>
    </div>
