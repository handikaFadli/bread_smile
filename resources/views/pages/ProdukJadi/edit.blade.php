    ...
    <div class="intro-y col-span-12 lg:col-span-8">
        ...
        <div class="intro-y box px-10 py-5">
            <form action="{{ route('produkJadi.update', $produkJadi->kd_produk) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input id="kd_produk" type="hidden" name="kd_produk" value="{{ $produkJadi->kd_produk }}">
                <div class="bg-slate-100 -ml-1 w-full flex justify-center py-2 rounded-lg shadow-lg">
                    <span class="text-slate-800 font-medium uppercase text-center">Kode Produk - {{ $produkJadi->kd_produk }}</span>
                </div>
                <div class="mt-8">
                    <label for="nm_produk" class="form-label">Nama Produk</label>
                    <input type="text" class="form-control w-full shadow-md @error('nm_produk') border-danger @enderror" id="nm_produk" name="nm_produk" value="{{ old('nm_produk', $produkJadi->nm_produk) }}" required>
                    @error('nm_produk')
                ...
                <div class="mt-6">
                    ...
                    <textarea class="form-control w-full shadow-md @error('ket') border-danger @enderror" id="ket" name="ket" required>{{ old('ket', $produkJadi->ket) }}</textarea>
                    @error('ket')
                    <div class="text-danger mt-2 mx-1">
                        {{ $message }}
                    </div>
